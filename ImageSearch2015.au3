...
;~  Func _WaitForImageSearchAreas($findImage, $waitSecs, $resultPosition, ByRef $x, ByRef $y, $tolerance, $transparency = 0)
;~ 	$waitSecs = $waitSecs * 1000
;~ 	$startTime = TimerInit()
;~ 	While TimerDiff($startTime) < $waitSecs
;~ 		For $i = 1 To $findImage[0]
...
;~ 	Return False
;~  EndFunc   ;==>_WaitForImagesSearch
 Func _WaitForImageSearchAreas($findImage, $waitSecs, $resultPosition, $x1, $y1, $right, $bottom, ByRef $x, ByRef $y, $tolerance = 0, $transparency = 0);Credits to Sven for the Transparency addition
    $waitSecs = $waitSecs * 1000
	$startTime = TimerInit()
	While TimerDiff($startTime) < $waitSecs
		For $i = 1 To $findImage[0]
		   Sleep(100)
			 If Not FileExists($findImage[0]) Then Return "Image File not found"
			 If $tolerance < 0 Or $tolerance > 255 Then $tolerance = 0
			 If $h_ImageSearchDLL = -1 Then _ImageSearchStartup()

			 If $transparency <> 0 Then $findImage[0] = "*" & $transparency & " " & $findImage
			 If $tolerance > 0 Then $findImage[0] = "*" & $tolerance & " " & $findImage
			 $result = DllCall($h_ImageSearchDLL, "str", "ImageSearch", "int", $x1, "int", $y1, "int", $right, "int", $bottom, "str", $findImage[0])
			 If @error Then Return "DllCall Error=" & @error
			 If $result = "0" Or Not IsArray($result) Or $result[0] = "0" Then Return False
